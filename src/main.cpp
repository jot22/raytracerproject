...
canHitGeneric *makeTriangleScene() {
    ...
    //shared_ptr<canHitGeneric> **list(new canHitGeneric*[2]);
    int i = 0;
    shared_ptr<texture> cT(constantTexture(vec3(1.0, 0.0, 0.0)));
    shared_ptr<material> red(diffuseMaterial_Lambertian(cT));
    ...
    list[i++] = translate(triangleCanHit(vec3(0, 0, -1), vec3(4, 0, -1), vec3(2, 2, -1), red),vec3(278,278,0));
...
//                                 new diffuseMaterial_Lambertian(new constantTexture(vec3(0.1, 0.2, 0.5))));
    ...
    return canHitGenericList(list, i);
}
